...
st.markdown("""
    ...
    .stTabs [data-baseweb="tab"] { color: #FFFFFF !important; font-size: 11px; padding: 0px 10px;}
    </style>
    """, unsafe_allow_html=True)

# --- BANCO DE DADOS ---
conn = sqlite3.connect("familybank_v13.db", check_same_thread=False)
cursor = conn.cursor()
...
with tab2:
    ...
    if not df_pago.empty:
        ...
        st.info("Liquide contas para ver os gráficos.")
    
    if st.button("📄 GERAR RELATÓRIO PDF"):
        pdf = FPDF()
        pdf.add_page()
        pdf.set_font("Arial", 'B', 16)
        pdf.cell(200, 10, f"Relatorio Mensal - {hoje.strftime('%m/%Y')}", ln=True, align='C')
        pdf_output = pdf.output(dest='S').encode('latin-1')
        st.download_button("📥 Baixar PDF", pdf_output, "relatorio.pdf", "application/pdf")

with tab5:
    st.markdown("<div class='section-title'>HISTÓRICO</div>", unsafe_allow_html=True)
    df_h = df_c[df_c['pago'] == 1].copy()
    if df_h.empty: st.write("Histórico vazio.")
    else:
...
with tab6:
    with st.form("add_v13", clear_on_submit=True):
        ...
        cat = st.selectbox("Categoria", ["Mercado", "Lazer", "Saúde", "Fixas", "Outros"])
