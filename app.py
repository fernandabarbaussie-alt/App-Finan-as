...
st.markdown("""
    ...
    .stButton>button { border: 2px solid #000000; border-radius: 8px; font-weight: 900; height: 3.5rem; background-color: #FFFFFF; color: #000000; box-shadow: 3px 3px 0px #000000;}
...
def acao_pagar(id_conta):
    ...
    cursor.execute("UPDATE contas SET pago = 1, data_pagamento = ? WHERE id = ?", (dt, id_conta))
    conn.commit()
    st.rerun()
