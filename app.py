...
# --- CONFIGURAÇÃO DA PÁGINA (ALTERAÇÃO DE NOME AQUI) ---
st.set_page_config(
...
# --- LOGICA DE ALERTAS (3 DIAS) ---
df_c = pd.read_sql("SELECT * FROM contas", conn)
df_i = pd.read_sql("SELECT * FROM investimentos", conn)
hoje = datetime.date.today()
proximos_dias = [(hoje + datetime.timedelta(days=i)).strftime("%d/%m") for i in range(4)]
alertas = df_c[(df_c['vencimento'].isin(proximos_dias)) & (df_c['pago'] == 0)]
...
with tab_dash:
    ...
    st.markdown("<br>#### Radar de Vencimentos", unsafe_allow_html=True)
            ...
            st.markdown(f"""
                <div class='expense-card'>
                    <div>
                        ...
                        <b>{r['descricao']}</b><br>
                        <small>Vence em: {r['vencimento']}</small>
                    </div>
                    <div style='color: #FF3366; font-weight: 800; font-size: 18px;'>R$ {r['valor']:,.2f}</div>
                </div>
            """, unsafe_allow_html=True)
    else:
        st.success("Nenhuma conta vencendo nos próximos dias! ✨")
...
with tab_lista:
    ...
    st.download_button("📤 Exportar para Excel", data=buffer.getvalue(), file_name="FamilyBank_Report.xlsx")
